import { actual_language } from './script';
import { articles, translation } from './registry';
import useStore from './stateManager';
const store = useStore();
let language;
const unsubscribeLanguage = store.subscribe('language', newLanguage => (language = newLanguage));
...
class IconButton extends HTMLElement {
	...
	connectedCallback() {
		...
		this.classList = 'demobox clickable';
		this.innerHTML = src == undefined ? '' : `<img loading="lazy" class="icon" src="${src}" alt="${text}">`;
		this.innerHTML += `<div class="center_text">${text}</div>`;
		if (redirect != undefined) this.addEventListener('click', () => store.modify('page', redirect));
	}
}
...
class ArticleCard extends HTMLElement {
	...
	connectedCallback() {
		...
		this.innerHTML = `
            <div class='flip_inner'>
                <div class='flip_front'>
                    ...
                    <div class='flip_button' value='${index}'>
                ...
                <div class='flip_back'>
                    <div class='read_button'>🔮 ${translation.enter[language]}</div>
        ...
        `;
		this.getElementsByClassName('flip_front')[0].addEventListener('click', () => store.modify('page', index));
		this.getElementsByClassName('read_button')[0].addEventListener('click', () => store.modify('page', index));
		for (let i = 0; i < 2; i++) {
			this.getElementsByClassName('flip_button')[i].addEventListener('mouseenter', function () {
				this.classList.add('hovered');
			});
			this.getElementsByClassName('flip_button')[i].addEventListener('mouseleave', function () {
				this.classList.remove('hovered');
			});
		}
		this.getElementsByClassName('flip_button')[0].addEventListener('click', async function (event) {
			event.stopPropagation();
			setTimeout(() => this.parentElement.parentElement.classList.add('hide'), 50); // prevent the bug on Safari
		...
		this.getElementsByClassName('flip_button')[1].addEventListener('click', function () {
			setTimeout(() => this.parentElement.parentElement.classList.remove('hide'), 125); // prevent the bug on Safari
