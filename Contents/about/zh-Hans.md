...
### V 2.6.0   2025年5月6日
1. 重构网站布局，将侧边导航面板与主内容区域包裹在亚克力背景中，并调整各面板部分之间的内边距，使背景圆角过渡更平滑；
2. 将文章卡片的浮动玻璃效果与悬停动画应用到所有符合条件的元素上，包括标题栏和按钮；
3. 标题栏能够根据位置动态改变颜色，以确保视觉上的舒适性；
4. 当用户访问网站时，网站的基本信息会输出至浏览器控制台；
5. 在翻转文章卡片时应用亮度渐变过渡效果；
6. 美化“桌椅评分”工具。
